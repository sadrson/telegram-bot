...
WEBHOOK_URL = os.getenv("RENDER_EXTERNAL_HOSTNAME")  # не обязательно, если используешь только уведомления
...
async def start(update: Update, context):
    ...
    print(f"Chat ID: {chat_id}")  # <- временный вывод в логи Render
    await update.message.reply_text(
        f"✅ Бот успешно работает! Твой chat_id: {chat_id}"
    )
